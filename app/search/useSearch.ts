...
export const useSearch = (query: string) => {
  ...
  const handleSearch = async () => {
    try {
      const [searchPhotoResponse, searchCollectionResponse] = await Promise.all([UnsplashService.search(CategoryEnum.PHOTO, {query}), await UnsplashService.search(CategoryEnum.COLLECTION, {query})])
